...
napi_value AsyncCall::Call(napi_env env, Context::ExecAction exec)
{
    if ((context_ == nullptr) || (context_->ctx == nullptr)) {
        DOWNLOAD_HILOGD("context_ or context_->ctx is null");
        return nullptr;
    }
    DOWNLOAD_HILOGD("async call exec");
...
{
    DOWNLOAD_HILOGD("run the async runnable");
...
{
    DOWNLOAD_HILOGD("run the js callback function");
    AsyncContext *context = reinterpret_cast<AsyncContext *>(data);
