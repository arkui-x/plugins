...
PauseNotify::PauseNotify(napi_env env, napi_value cb, Version version, Action action)
    : JsNotify(env, cb, version, action)
{
}

void PauseNotify::HandleCallback(napi_env env, napi_value cb, const std::string& param)
{
    REQUEST_HILOGE("PauseNotify::HandleCallback start");
    ...
    if (version_ == Version::API9) {
        if (action_ == Action::UPLOAD) {
            REQUEST_HILOGE("PauseNotify can't support upload");
            return;
        }
    ...
    REQUEST_HILOGI("PauseNotify::HandleCallback end");
}
}