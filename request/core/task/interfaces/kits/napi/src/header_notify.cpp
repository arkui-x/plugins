...
HeaderNotify::HeaderNotify(napi_env env, napi_value cb, Version version, Action action)
    : JsNotify(env, cb, version, action)
{
}

void HeaderNotify::HandleCallback(napi_env env, napi_value cb, const std::string& param)
{
    REQUEST_HILOGE("HeaderNotify::HandleCallback start");
    ...
    if (version_ != Version::API9 || action_ != Action::UPLOAD) {
        REQUEST_HILOGE("upload v9 support header received event");
        return;
    }
    ...
    REQUEST_HILOGE("HeaderNotify::HandleCallback end");
}
}