...
ResumeNotify::ResumeNotify(napi_env env, napi_value cb, Version version, Action action)
    : JsNotify(env, cb, version, action)
{
}

void ResumeNotify::HandleCallback(napi_env env, napi_value cb, const std::string& param)
{
    REQUEST_HILOGE("ResumeNotify::HandleCallback start");
    ...
    if (version_ == Version::API9) {
        if (action_ == Action::UPLOAD) {
            REQUEST_HILOGE("ResumeNotify can't support upload");
            return;
        }
    ...
    REQUEST_HILOGI("ResumeNotify::HandleCallback end");
}
}