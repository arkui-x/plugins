    ...
    const std::string& fieldStr, std::shared_ptr<GeoAddress> address)
{
    ...
    size_t size = 0;
    if (address->descriptionsSize_ >= 0) {
        size = std::min(static_cast<size_t>(address->descriptionsSize_), descriptions.size());
    } else {
        size = 0;
    }
...
{
    if (fieldStr == nullptr || str == nullptr) {
        LBSLOGE(LOCATOR_STANDARD, "SetValueUtf8String: fieldStr or str is nullptr");
