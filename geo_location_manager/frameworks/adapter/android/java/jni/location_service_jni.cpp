...
const char LOCATION_SERVICE_CLASS_NAME[] = "ohos/ace/plugin/locationserviceplugin/LocationServicePlugin";
...
static inline std::string JStringToCpp(JNIEnv* env, jobject jobjStr)
{
    ...
    const char* chars = env->GetStringUTFChars((jstring)jobjStr, nullptr);
    if (!chars) {
        return "";
    }
    std::string out(chars);
    env->ReleaseStringUTFChars((jstring)jobjStr, chars);
    return out;
}
    ...
    std::list<std::shared_ptr<Location::GeoAddress>>& replyList)
{
    ...
    auto grabStr = [env](jobject js) -> std::string {
        ...
        const char* chars = env->GetStringUTFChars((jstring)js, nullptr);
        std::string str(chars ? chars : "");
        if (chars) {
            env->ReleaseStringUTFChars((jstring)js, chars);
        }
        return str;
    };
...
void LocationServiceJni::NativeOnCountryCodeChanged(JNIEnv *env, jobject, jstring jCode, jint type)
{
    ...
    std::string code;
    if (jCode) {
        const char* chars = env->GetStringUTFChars(jCode, nullptr);
        if (chars) {
            code.assign(chars);
...
void LocationServiceJni::NativeOnNmeaMessage(JNIEnv *env, jobject, jlong timestamp, jstring jSentence)
{
    ...
    std::string sentence;
    if (jSentence) {
        const char *chars = env->GetStringUTFChars(jSentence, nullptr);
        if (chars) {
            sentence.assign(chars);
            env->ReleaseStringUTFChars(jSentence, chars);
        }
    }
    for (auto &cb : list) {
        if (cb) {
    ...
    jstring jDeviceId, jstring jDeviceName, jint jRssi, jbyteArray jData, jboolean jConnectable)
{
    ...
    std::string devName;
    if (jDeviceId) {
        const char *devIdChars = env->GetStringUTFChars(jDeviceId, nullptr);
        if (devIdChars) {
            devId = devIdChars;
            env->ReleaseStringUTFChars(jDeviceId, devIdChars);
        }
    }
    if (jDeviceName) {
        const char *devNameChars = env->GetStringUTFChars(jDeviceName, nullptr);
        if (devNameChars) {
            devName = devNameChars;
            env->ReleaseStringUTFChars(jDeviceName, devNameChars);
        }
    }
