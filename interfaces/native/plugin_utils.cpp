...
void ARKUI_X_Plugin_RunAsyncTask(ARKUI_X_Plugin_Task task, ARKUI_X_Plugin_Thread_Mode mode)
{
    ...
    if (taskExecutor) {
        if (mode == ARKUI_X_PLUGIN_PLATFORM_THREAD) {
            taskExecutor->PostTask([task]() { task(); },
                OHOS::Ace::TaskExecutor::TaskType::PLATFORM, "ArkUI-XPluginUtilsRunAsyncTaskPlatform");
        } else if (mode == ARKUI_X_PLUGIN_JS_THREAD) {
            taskExecutor->PostTask([task]() { task(); },
                OHOS::Ace::TaskExecutor::TaskType::JS, "ArkUI-XPluginUtilsRunAsyncTaskJs");
        } else {
