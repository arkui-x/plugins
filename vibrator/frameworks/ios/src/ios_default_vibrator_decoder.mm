...
bool IOSDefaultVibratorDecoder::CheckEventParameters(const VibrateEvent &event)
{
    ...
    if (event.tag == EVENT_TAG_CONTINUOUS) {
        constexpr int32_t CONTINUOUS_VIBRATION_DURATION_MAX = 5000;
        if (event.duration > CONTINUOUS_VIBRATION_DURATION_MAX) {
            NSLog(@"Continuous event duration out of range: %d", event.duration);
            return false;
        }
    }
    ...
    int32_t eventDuration, VibrateCurvePoint &point)
{
    ...
    if (point.frequency < CURVE_FREQUENCY_MIN || point.frequency > CURVE_FREQUENCY_MAX) {
        NSLog(@"The frequency of curve point is out of bounds, frequency:%d", point.frequency);
        return ERROR;
    }
    return SUCCESS;
}
