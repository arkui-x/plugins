...
bool IosJsonParser::HasObjectItem(const void* parent, const std::string &key) const
{
    @autoreleasepool {
        if (!parent) {
            NSLog(@"HasObjectItem: parent is null");
            return false;
        }
        id parentObj = (__bridge id)parent;
        if (![parentObj isKindOfClass:[NSDictionary class]]) {
            NSLog(@"HasObjectItem: parent is not a dictionary");
            return false;
        }
        NSString *keyStr = [NSString stringWithUTF8String:key.c_str()];
        if (!keyStr) {
            NSLog(@"HasObjectItem: failed to convert key to NSString");
            return false;
        }
        id result = [(NSDictionary *)parentObj objectForKey:keyStr];
