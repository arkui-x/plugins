...
int32_t IOSVibratorDecoder::ParseEvent(const IosJsonParser &parser, void* eventJSON, VibrateEvent &event)
{
    ...
    if (parser.HasObjectItem(eventJSON, "Index")) {
        void* indexJSON = parser.GetObjectItem(eventJSON, "Index");
        if (indexJSON == nullptr) {
            NSLog(@"Index not found in event");
            return ERROR;
        }
        event.index = parser.GetIntValue(indexJSON, -1);
    } else {
        event.index = 0;
    }
...
bool IOSVibratorDecoder::CheckCommonParameters(const VibrateEvent &event)
{
    if (event.time < 0) {
    ...
    if (event.index < INDEX_MIN || event.index > INDEX_MAX) {
        NSLog(@"Invalid event index: %d", event.index);
        return false;
    }
...
bool IOSVibratorDecoder::CheckTransientParameters(const VibrateEvent &event)
{
    if (event.frequency < TRANSIENT_FREQUENCY_MIN || event.frequency > TRANSIENT_FREQUENCY_MAX) {
        NSLog(@"Invalid frequency for transient event: %d", event.frequency);
        return false;
    }
    return true;
}

bool IOSVibratorDecoder::CheckContinuousParameters(const VibrateEvent &event)
{
    ...
    if (pointNum < CURVE_POINT_NUM_MIN || pointNum > CURVE_POINT_NUM_MAX) {
        NSLog(@"Invalid curve points count: %d", pointNum);
        return false;
    }
...
int32_t IOSVibratorDecoder::ParseCurve(const IosJsonParser &parser, void* curveJSON, VibrateEvent &event)
{
    ...
    for (int32_t i = 0; i < size; i++) {
        ...
        if (point.time <= previousCurveTime) {
            NSLog(@"Invalid curve point time: %d, previous: %d", point.time, previousCurveTime);
            return ERROR;
        }
    ...
    if (!event.points.empty()) {
        if (event.points[0].time != 0 || event.points[0].intensity != 0) {
            NSLog(@"The first curve point is invalid, time:%d, intensity:%d", 
                  event.points[0].time, event.points[0].intensity);
            return ERROR;
        }
        if (event.points.back().time != event.duration || event.points.back().intensity != 0) {
            NSLog(@"The last curve point is invalid, time:%d, intensity:%d", 
                  event.points.back().time, event.points.back().intensity);
            return ERROR;
        }
    }
