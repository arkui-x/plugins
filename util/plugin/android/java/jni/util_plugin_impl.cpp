...
#include "plugins/util/plugin/android/java/jni/util_plugin_impl.h"
...
{
    return UtilPluginJni::EncodeIntoChinese(input, encoding);
}

std::string UtilPluginImpl::Decode(std::string input, std::string encoding)
{
    return UtilPluginJni::Decode(input, encoding);
}
} // namespace OHOS::Plugin